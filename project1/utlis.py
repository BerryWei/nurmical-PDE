...
class ODESolver:
    def __init__(self, rhs, dt, t_span, y0, solver='hybird', jac = None) -> None:
        ...
        self.dir_solvers = {
            'hybird': root,
            'newton': self.newton_raphson,
            'moore_penrose': self.moore_penrose
        }
    ...
    def solve(self, **kwargs):
        ...
        for i in range(self.n_steps):
            ...
            t_next = (i + 1) * self.dt
            # find root
            if self.solver == 'newton' or self.solver == 'moore_penrose':
                res = solver_fn(self.rhs, y_prev)
            else:
    ...
    @staticmethod
    def newton_raphson(fun : callable, x0 : np.ndarray, eps=1e-6, max_iter=100):
